...
<div id="stockModal-{{ $product_stock->product_id }}" class="fixed inset-0 z-50 hidden overflow-y-auto" role="dialog" aria-modal="true" aria-labelledby="modal-title">
            ...
            <div class="bg-gray-300 px-6 py-5">
                <div class="flex items-center mb-4">
                    ...
                    <h3 class="ml-3 text-lg font-semibold text-gray-800" id="modal-title">
                        Add Stock to <span class="text-indigo-600">{{ $product_stock->product_name }}</span>
                    </h3>
                </div>

                <form method="POST" action="{{ route('product.addStock', $product_stock->product_id) }}">
                    @csrf
                    ...
                    <div class="mt-6 flex justify-end space-x-3">
                        <button type="button"
                                ...
                                onclick="document.getElementById('stockModal-{{ $product_stock->product_id }}').classList.add('hidden')">
                            Cancel
                        </button>
                        <button type="submit"
